...
func animate(source, owner: CombatCard, icon_to_animate, params):
	...
	if icon_to_animate and animate_scale:
		...
		tween.tween_property(icon_to_animate, "scale", Vector2.ONE * 5.0, duration)
		tween.tween_property(icon_to_animate, "position", Vector2(14, -85), duration)
		tween.set_ease(Tween.EASE_OUT)
		tween.set_parallel(false)
		tween.tween_property(icon_to_animate, "scale", Vector2.ONE, duration)
		tween.set_parallel(true)
		tween.tween_property(icon_to_animate, "position", icon_to_animate.origin_position, duration)
